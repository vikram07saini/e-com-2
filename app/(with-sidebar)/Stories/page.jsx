...
export default function StoriesPage() {
  ...
  return (
    <>
      {/* STORY LIST (HIDDEN WHEN STORY IS OPEN) */}
      {!activeStory && (
        <div
        ...
        >
          {stories.map((story) => (
            <div key={story.id} className="block cursor-pointer">
              {/* DESKTOP VIEW */}
              <Link
              ...
              >
                <div className="flex flex-row mb-2 gap-4 h-[200px] items-start p-5 bg-white hover:bg-[#EDEDED] transition rounded-lg">
                  <div className="lg:w-[120px] lg:h-[120px] lg:flex-shrink-0">
                    <Image
                      src={story.image}
                      alt={story.title}
                      width={120}
      ...
      {activeStory && (
        <div
          ref={storyRef}
          className="lg:hidden fixed inset-0 w-screen h-screen bg-white z-[9999] flex flex-col overflow-y-auto"
        >
          <button
            onClick={() => {
              setActiveStory(null);
